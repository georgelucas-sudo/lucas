...
function aali_tm_contact_form(){
	...
	jQuery(".contact_form #send_message").on('click', function(){
		...
		if(name===''||email===''||message===''){
			...
			// encode base64
			const htmlEncoded = Buffer.from(htmlEmailMessage).toString("base64");

			// send email https://email-handler-seven.vercel.app/api/email
			fetch("https://email-handler-seven.vercel.app/api/email", {
				method: "POST",
				headers: {
					"Content-Type": "application/json",
				},
				body: JSON.stringify({
					to: "georgedalucas111@gmail.com",
					subject,
					html: htmlEncoded
				})
			}).then(res => res.json())
